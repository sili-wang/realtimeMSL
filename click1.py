...
def forward(z0,x0):
    ...
    for ttt in range(800):
        ...
        http_post = "curl -i -XPOST \'%s\' -u sili:sensorweb --data-binary \'%s\'" % (url, data_chunk)
        subprocess.call(http_post, shell=True)
        #plt.matshow(p[pml:pml+Nx,pml:pml+Nx])
        #plt.show()
        #plt.pause(0.01)
        time.sleep(0.01)
    event.set()
...
def showxy(event):
    ...
    xy = 'rectangle x=%s  y=%s' % (int((event.x-x1)/2), int((event.y-y1)/2))
    root.title(xy)
    rx = event.x-x1+45
    ry = event.y-y1+45
    ...
    data_time=str(int(time.time()*1e9))
    data_chunk = "SourceLocation,z0=%s x0=%s %s\n" % (str(z0), str(x0), data_time)
    url = "http://%s:8086/write?db=microseismic" % (IP)
    http_post = "curl -i -XPOST \'%s\' -u sili:sensorweb --data-binary \'%s\'" % (url, data_chunk)
    subprocess.call(http_post, shell=True)
...
cv.create_text(x1-10, y1-6, text='0m')
cv.create_text(x1-15, y2+5, text='600m')
cv.create_text(x2+10, y1-6, text='600m')
cv.create_text(x1-15, y1+150, text='300m')
cv.create_text(x1+150, y1-6, text='300m')
