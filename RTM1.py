...
while aa:
    ...
    for b in range(pml,pml+Nx,1):
        query = 'SELECT "value" FROM Amplitude WHERE "location" = \'R'+str(b)+'\' and time > \''+DT+'\'+235m and time < \''+DT+'\'+4h  ' 
        result = client.query(query)
        points = list(result.get_points())
        value =  np.append(value,np.array(list(map(operator.itemgetter('value'), points))))
    length = len(points)
    value=np.reshape(value, (Nx,-1))
#    aa=0
#times  =  map(operator.itemgetter('time'),  points)
    ...
    for ttt in range(length):
        p[pml:pml+Nx:1,pml]=value[0:Nx,length-ttt-1]
    ...
    plt.imshow(np.transpose(em[pml:pml+Nx,pml:pml+Nx]), extent=(0,600,600,0))
